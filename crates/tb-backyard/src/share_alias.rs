//! Pure logic for the `tb-backyard share alias` CLI subcommands. Validation,
...
//! `BackyardClient`) sits in `main.rs` next to `share_upload`.
//!
//! The slug rules MUST stay in lockstep with the server's
//! `ShareAlias::SLUG_REGEX` and `RESERVED_SLUGS`
//! (backyard `app/models/share_alias.rb`). The parity unit test in
