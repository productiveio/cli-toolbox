...
//! `backyard/spec/models/share_alias_spec.rb` exactly. Any diff here
//! signals drift between the server's `ShareAlias::SLUG_REGEX` /
//! `RESERVED_SLUGS` and the CLI port in `tb_backyard::share_alias`.

use tb_backyard::share_alias::{RESERVED_SLUGS, normalize_slug, validate_slug};
...
fn reserved_list_matches_server() {
    // Mirror of `ShareAlias::RESERVED_SLUGS` in backyard
