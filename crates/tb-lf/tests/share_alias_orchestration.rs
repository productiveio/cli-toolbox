//! Orchestration logic for `tb-lf share alias set/list/rm`. The HTTP layer
...
use tb_lf::share_alias::{OptInGate, opt_in_gate, parse_share_target};
...
fn parses_localhost_url() {
    // The CLI hits whatever `devportal_url` is configured — accept
