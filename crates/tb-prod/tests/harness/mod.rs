...
fn load_credentials() -> (String, String) {
    // Try env vars first
    if let (Ok(token), Ok(org)) = (
        std::env::var("TB_PROD_TOKEN"),
        std::env::var("TB_PROD_ORG"),
    ) {
        return (token, org);
    }
    ...
    let workspace_secrets = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../secrets.toml");
        ...
        if let Ok(content) = std::fs::read_to_string(&workspace_secrets) {
            if let Ok(table) = content.parse::<toml::Table>() {
                if let Some(section) = table.get("productive").and_then(|v| v.as_table()) {
                    ...
                    let org = section.get("org_id").and_then(|v| v.as_str()).unwrap_or("10");
                    if !token.is_empty() {
