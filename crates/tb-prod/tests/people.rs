...
async fn cache_name_resolution() {
    ...
    let resp = client
        .get_page("/people", &Query::new(), 1, 20)
        .await
        .unwrap();
    ...
    let records: Vec<tb_prod::generic_cache::CachedRecord> = resp
        .data
        .iter()
        .map(|r| {
