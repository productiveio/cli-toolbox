...
pub fn exit_with_tb_error(err: &crate::error::TbProdError) -> ! {
    match err {
        crate::error::TbProdError::Api { status, message } => exit_with_error_details(
            "api_error",
            &format!("API error ({})", status),
            Some(json!({ "status": status, "body": message })),
        ),
        crate::error::TbProdError::Config(msg) => exit_with_error("config_error", msg),
        other => exit_with_error("internal_error", &other.to_string()),
    }
}
